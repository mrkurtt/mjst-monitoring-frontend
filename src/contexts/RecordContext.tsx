...
export const RecordProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateManuscriptStatus = (
    ...
    if (status === 'pre-review' && details?.revisionStatus === 'For Revision') {
      setManuscriptRecords(prev => prev.map(manuscript => 
        manuscript.id === id ? {
          ...manuscript,
          ...details,
          status: 'pre-review'
  ...
  const fetchPublishedRecords = async () => {
    try {
      ...
      const enrichedData = await Promise.all(data.map(async (record: any) => {
        try {
          const staffPublishedRes = await fetch(`/api/staff-published/${record.id}`);
          const staffPublished = await staffPublishedRes.json();
          console.log(`Staff Published Data for ${record.id}:`, staffPublished);
          ...
          console.error(`Error fetching staff published data for record ${record.id}:`, error);
          return record;
        }
      }));
