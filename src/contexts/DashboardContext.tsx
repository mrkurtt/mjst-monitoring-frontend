...
export const DashboardProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const calculateMonthlySubmissions = useCallback(() => {
    ...
    console.log('All Records:', allRecords.map(record => ({
      date: record.date,
    ...
    allRecords.forEach(record => {
      if (record.date) {  
        ...
        if (year === selectedYear) {
          ...
          monthlyData[month]++;
          console.log(`Counting submission for ${record.date}, Month: ${month}, Year: ${year}`);
        }
      } else {
  ...
  const calculateStats = useCallback(() => {
    ...
    const newStats = {
      uploadCount: manuscriptRecords.length + doubleBlindRecords.length + acceptedRecords.length + finalProofreadingRecords.length + publishedRecords.length + rejectedRecords.length,
