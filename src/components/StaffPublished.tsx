...
const StaffPublished: React.FC = () => {
	...
	const filteredRecords = publishedRecords.filter((record) => {
		...
		const matchesIssue =
			sortIssue === 'all' || record.publishDetails?.scopeNumber === sortIssue;
	...
	const getManuscripts = async () => {
		await getManuscriptByStepStatus('Published').then((res) =>
						...
						{searchTerm === ''
							? manuscripts.map((record: any, index: number) => (
									<tr key={index} className="hover:bg-gray-50">
										...
										<td className="py-4 px-4">
											{moment(record.datePublished).format('LL')}
										</td>
										<td className="py-4 px-4">
											<button
								...
								<div className="bg-gray-50 p-4 rounded-lg mb-6">
									...
									<div className="grid grid-cols-2 gap-4">
										...
										<div>
											...
											<p className="text-gray-700">
												{moment(selectedRecord.datePublished).format('LL')}
											</p>
										</div>
									</div>
								</div>
								...
								<div className="bg-gray-50 p-4 rounded-lg">
									...
									<div className="grid grid-cols-2 gap-4">
										...
										<div>
											...
											<p className="text-gray-700">
												{moment(selectedRecord.dateSent).format('LL')}
											</p>
										</div>
									</div>
								</div>
